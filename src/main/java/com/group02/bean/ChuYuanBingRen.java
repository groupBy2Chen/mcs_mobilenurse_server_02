package com.group02.bean;

import java.sql.Date;
import java.util.List;

/**
 * ChuYuanBingRen 具体类
 */
public class ChuYuanBingRen {

    /**
     * 封装树形控件所需要的属性
     */
    String label;

    /**
     * 封装树形控件所需要的属性
     */
    List<ChuYuanBingRen> children;

    /**
     * 系统ID
     */
    String xiTongID;

    /**
     *病人ID
     */
    String bingRenID;

    /**
     *是否被改变
     */
    Integer beiGaiBian;

    /**
     *上级ID
     */
    String shangJiID;

    /**
     *更新时间
     */
    Date gengXinShiJian;

    /**
     *病人信息
     */
    String bingRenXinXi;

    /**
     *床号
     */
    String chuangHao;

    /**
     *临床路径
     */
    String linChuangLuJing;

    /**
     *出生日期
     */
    Date chuShengRiQi;

    /**
     *费别
     */
    String feiBie;

    /**
     * 过敏史
     */
    String guoMinShi;

    /**
     *护理等级
     */
    String huLiDengJi;

    /**
     *婚姻状况
     */
    String hunYinZhuangKuang;

    /**
     *既往史
     */
    String jiWangShi;

    /**
     *科室ID
     */
    String keShiID;

    /**
     *备用01
     */
    String beiYong01;

    /**
     *科室名称
     */
    String keShiMingCheng;

    /**
     *标记
     */
    String biaoJi;

    /**
     * 年龄
     */
    String nianLing;

    /**
     * 欠费金额
     */
    String qianFeiJinE;

    /**
     * 入科时间
     */
    String ruKeShiJian;

    /**
     * 如科天数
     */
    String ruKeTianShu;

    /**
     * 入科小时
     */
    String ruKeXiaoShi;

    /**
     * 入院时间
     */
    String ruYuanShiJian;

    /**
     * 入院天数
     */
    String ruYuanTianShu;

    /**
     *入院小时
     */
    String ruYuanXiaoShi;

    /**
     *被探望ID
     */
    String beiTanWangID;

    /**
     *性别
     */
    String xingBie;

    /**
     *姓名
     */
    String xingMing;

    /**
     *信息
     */
    String xinXi;

    /**
     *预交金额
     */
    String yuJiaoJinE;

    /**
     * 责任护士
     */
    String zeRenHuShi;

    /**
     *自费费用
     */
    String ziFeiFeiYong;

    /**
     *诊断
     */
    String zhenDuan;

    /**
     *主治医生
     */
    String zhuZhiYiSheng;

    /**
     *自交费用
     */
    String ziJiaoFeiYong;

    /**
     * 状态
     */
    String zhuangTai;

    /**
     * 住院
     */
    String zhuYuan;

    /**
     * 住院号码
     */
    String zhuYuanHaoMa;

    /**
     * 住院状态
     */
    String zhuYuanZhuangTai;

    /**
     * 联系电话
     */
    String lianXiDianHua;

    /**
     *联系人姓名
     */
    String lianXiRenXingMing;

    /**
     *与患者关系
     */
    String yuHuanZheGuanXi;

    public String getLabel() {
        return label;
    }

    public void setLabel(String label) {
        this.label = label;
    }

    public List<ChuYuanBingRen> getChildren() {
        return children;
    }

    public void setChildren(List<ChuYuanBingRen> children) {
        this.children = children;
    }

    public String getXiTongID() {
        return xiTongID;
    }

    public void setXiTongID(String xiTongID) {
        this.xiTongID = xiTongID;
    }

    public String getBingRenID() {
        return bingRenID;
    }

    public void setBingRenID(String bingRenID) {
        this.bingRenID = bingRenID;
    }

    public Integer getBeiGaiBian() {
        return beiGaiBian;
    }

    public void setBeiGaiBian(Integer beiGaiBian) {
        this.beiGaiBian = beiGaiBian;
    }

    public String getShangJiID() {
        return shangJiID;
    }

    public void setShangJiID(String shangJiID) {
        this.shangJiID = shangJiID;
    }

    public Date getGengXinShiJian() {
        return gengXinShiJian;
    }

    public void setGengXinShiJian(Date gengXinShiJian) {
        this.gengXinShiJian = gengXinShiJian;
    }

    public String getBingRenXinXi() {
        return bingRenXinXi;
    }

    public void setBingRenXinXi(String bingRenXinXi) {
        this.bingRenXinXi = bingRenXinXi;
    }

    public String getChuangHao() {
        return chuangHao;
    }

    public void setChuangHao(String chuangHao) {
        this.chuangHao = chuangHao;
    }

    public String getLinChuangLuJing() {
        return linChuangLuJing;
    }

    public void setLinChuangLuJing(String linChuangLuJing) {
        this.linChuangLuJing = linChuangLuJing;
    }

    public Date getChuShengRiQi() {
        return chuShengRiQi;
    }

    public void setChuShengRiQi(Date chuShengRiQi) {
        this.chuShengRiQi = chuShengRiQi;
    }

    public String getFeiBie() {
        return feiBie;
    }

    public void setFeiBie(String feiBie) {
        this.feiBie = feiBie;
    }

    public String getGuoMinShi() {
        return guoMinShi;
    }

    public void setGuoMinShi(String guoMinShi) {
        this.guoMinShi = guoMinShi;
    }

    public String getHuLiDengJi() {
        return huLiDengJi;
    }

    public void setHuLiDengJi(String huLiDengJi) {
        this.huLiDengJi = huLiDengJi;
    }

    public String getHunYinZhuangKuang() {
        return hunYinZhuangKuang;
    }

    public void setHunYinZhuangKuang(String hunYinZhuangKuang) {
        this.hunYinZhuangKuang = hunYinZhuangKuang;
    }

    public String getJiWangShi() {
        return jiWangShi;
    }

    public void setJiWangShi(String jiWangShi) {
        this.jiWangShi = jiWangShi;
    }

    public String getKeShiID() {
        return keShiID;
    }

    public void setKeShiID(String keShiID) {
        this.keShiID = keShiID;
    }

    public String getBeiYong01() {
        return beiYong01;
    }

    public void setBeiYong01(String beiYong01) {
        this.beiYong01 = beiYong01;
    }

    public String getKeShiMingCheng() {
        return keShiMingCheng;
    }

    public void setKeShiMingCheng(String keShiMingCheng) {
        this.keShiMingCheng = keShiMingCheng;
    }

    public String getBiaoJi() {
        return biaoJi;
    }

    public void setBiaoJi(String biaoJi) {
        this.biaoJi = biaoJi;
    }

    public String getNianLing() {
        return nianLing;
    }

    public void setNianLing(String nianLing) {
        this.nianLing = nianLing;
    }

    public String getQianFeiJinE() {
        return qianFeiJinE;
    }

    public void setQianFeiJinE(String qianFeiJinE) {
        this.qianFeiJinE = qianFeiJinE;
    }

    public String getRuKeShiJian() {
        return ruKeShiJian;
    }

    public void setRuKeShiJian(String ruKeShiJian) {
        this.ruKeShiJian = ruKeShiJian;
    }

    public String getRuKeTianShu() {
        return ruKeTianShu;
    }

    public void setRuKeTianShu(String ruKeTianShu) {
        this.ruKeTianShu = ruKeTianShu;
    }

    public String getRuKeXiaoShi() {
        return ruKeXiaoShi;
    }

    public void setRuKeXiaoShi(String ruKeXiaoShi) {
        this.ruKeXiaoShi = ruKeXiaoShi;
    }

    public String getRuYuanShiJian() {
        return ruYuanShiJian;
    }

    public void setRuYuanShiJian(String ruYuanShiJian) {
        this.ruYuanShiJian = ruYuanShiJian;
    }

    public String getRuYuanTianShu() {
        return ruYuanTianShu;
    }

    public void setRuYuanTianShu(String ruYuanTianShu) {
        this.ruYuanTianShu = ruYuanTianShu;
    }

    public String getRuYuanXiaoShi() {
        return ruYuanXiaoShi;
    }

    public void setRuYuanXiaoShi(String ruYuanXiaoShi) {
        this.ruYuanXiaoShi = ruYuanXiaoShi;
    }

    public String getBeiTanWangID() {
        return beiTanWangID;
    }

    public void setBeiTanWangID(String beiTanWangID) {
        this.beiTanWangID = beiTanWangID;
    }

    public String getXingBie() {
        return xingBie;
    }

    public void setXingBie(String xingBie) {
        this.xingBie = xingBie;
    }

    public String getXingMing() {
        return xingMing;
    }

    public void setXingMing(String xingMing) {
        this.xingMing = xingMing;
    }

    public String getXinXi() {
        return xinXi;
    }

    public void setXinXi(String xinXi) {
        this.xinXi = xinXi;
    }

    public String getYuJiaoJinE() {
        return yuJiaoJinE;
    }

    public void setYuJiaoJinE(String yuJiaoJinE) {
        this.yuJiaoJinE = yuJiaoJinE;
    }

    public String getZeRenHuShi() {
        return zeRenHuShi;
    }

    public void setZeRenHuShi(String zeRenHuShi) {
        this.zeRenHuShi = zeRenHuShi;
    }

    public String getZiFeiFeiYong() {
        return ziFeiFeiYong;
    }

    public void setZiFeiFeiYong(String ziFeiFeiYong) {
        this.ziFeiFeiYong = ziFeiFeiYong;
    }

    public String getZhenDuan() {
        return zhenDuan;
    }

    public void setZhenDuan(String zhenDuan) {
        this.zhenDuan = zhenDuan;
    }

    public String getZhuZhiYiSheng() {
        return zhuZhiYiSheng;
    }

    public void setZhuZhiYiSheng(String zhuZhiYiSheng) {
        this.zhuZhiYiSheng = zhuZhiYiSheng;
    }

    public String getZiJiaoFeiYong() {
        return ziJiaoFeiYong;
    }

    public void setZiJiaoFeiYong(String ziJiaoFeiYong) {
        this.ziJiaoFeiYong = ziJiaoFeiYong;
    }

    public String getZhuangTai() {
        return zhuangTai;
    }

    public void setZhuangTai(String zhuangTai) {
        this.zhuangTai = zhuangTai;
    }

    public String getZhuYuan() {
        return zhuYuan;
    }

    public void setZhuYuan(String zhuYuan) {
        this.zhuYuan = zhuYuan;
    }

    public String getZhuYuanHaoMa() {
        return zhuYuanHaoMa;
    }

    public void setZhuYuanHaoMa(String zhuYuanHaoMa) {
        this.zhuYuanHaoMa = zhuYuanHaoMa;
    }

    public String getZhuYuanZhuangTai() {
        return zhuYuanZhuangTai;
    }

    public void setZhuYuanZhuangTai(String zhuYuanZhuangTai) {
        this.zhuYuanZhuangTai = zhuYuanZhuangTai;
    }

    public String getLianXiDianHua() {
        return lianXiDianHua;
    }

    public void setLianXiDianHua(String lianXiDianHua) {
        this.lianXiDianHua = lianXiDianHua;
    }

    public String getLianXiRenXingMing() {
        return lianXiRenXingMing;
    }

    public void setLianXiRenXingMing(String lianXiRenXingMing) {
        this.lianXiRenXingMing = lianXiRenXingMing;
    }

    public String getYuHuanZheGuanXi() {
        return yuHuanZheGuanXi;
    }

    public void setYuHuanZheGuanXi(String yuHuanZheGuanXi) {
        this.yuHuanZheGuanXi = yuHuanZheGuanXi;
    }

    @Override
    public String toString() {
        return "ChuYuanBingRen{" +
                "label='" + label + '\'' +
                ", children=" + children +
                ", xiTongID='" + xiTongID + '\'' +
                ", bingRenID='" + bingRenID + '\'' +
                ", beiGaiBian=" + beiGaiBian +
                ", shangJiID='" + shangJiID + '\'' +
                ", gengXinShiJian=" + gengXinShiJian +
                ", bingRenXinXi='" + bingRenXinXi + '\'' +
                ", chuangHao='" + chuangHao + '\'' +
                ", linChuangLuJing='" + linChuangLuJing + '\'' +
                ", chuShengRiQi=" + chuShengRiQi +
                ", feiBie='" + feiBie + '\'' +
                ", guoMinShi='" + guoMinShi + '\'' +
                ", huLiDengJi='" + huLiDengJi + '\'' +
                ", hunYinZhuangKuang='" + hunYinZhuangKuang + '\'' +
                ", jiWangShi='" + jiWangShi + '\'' +
                ", keShiID='" + keShiID + '\'' +
                ", beiYong01='" + beiYong01 + '\'' +
                ", keShiMingCheng='" + keShiMingCheng + '\'' +
                ", biaoJi='" + biaoJi + '\'' +
                ", nianLing='" + nianLing + '\'' +
                ", qianFeiJinE='" + qianFeiJinE + '\'' +
                ", ruKeShiJian='" + ruKeShiJian + '\'' +
                ", ruKeTianShu='" + ruKeTianShu + '\'' +
                ", ruKeXiaoShi='" + ruKeXiaoShi + '\'' +
                ", ruYuanShiJian='" + ruYuanShiJian + '\'' +
                ", ruYuanTianShu='" + ruYuanTianShu + '\'' +
                ", ruYuanXiaoShi='" + ruYuanXiaoShi + '\'' +
                ", beiTanWangID='" + beiTanWangID + '\'' +
                ", xingBie='" + xingBie + '\'' +
                ", xingMing='" + xingMing + '\'' +
                ", xinXi='" + xinXi + '\'' +
                ", yuJiaoJinE='" + yuJiaoJinE + '\'' +
                ", zeRenHuShi='" + zeRenHuShi + '\'' +
                ", ziFeiFeiYong='" + ziFeiFeiYong + '\'' +
                ", zhenDuan='" + zhenDuan + '\'' +
                ", zhuZhiYiSheng='" + zhuZhiYiSheng + '\'' +
                ", ziJiaoFeiYong='" + ziJiaoFeiYong + '\'' +
                ", zhuangTai='" + zhuangTai + '\'' +
                ", zhuYuan='" + zhuYuan + '\'' +
                ", zhuYuanHaoMa='" + zhuYuanHaoMa + '\'' +
                ", zhuYuanZhuangTai='" + zhuYuanZhuangTai + '\'' +
                ", lianXiDianHua='" + lianXiDianHua + '\'' +
                ", lianXiRenXingMing='" + lianXiRenXingMing + '\'' +
                ", yuHuanZheGuanXi='" + yuHuanZheGuanXi + '\'' +
                '}';
    }
}
